...
export type ChangesCallback<T> = (c: Change<T>[]) => void;
export type ChangeType = 'update' | 'delete' | 'add';
...
export const ObservableSlim = (function() {
  var onModelChanging: (c: Change<any>) => Observable<boolean>;
  var paths = [];
  ...
  /*	Function: _create
			...
			Returns:
				An ES6 Proxy object.
	*/
  var _create = function<T>(target, domDelay, originalObservable, originalPath) {
    ...
    /*	Function: _getPath
		...
		*/
    var _getPath = function(target, property, jsonPointer = false) {
      var fullPath = '';
      var lastTarget = null;
    ...
    var _doSet = function(originalChange, property, value, typeOfTargetProp, targetProp) {
    ...
    var handler = {
      ...
      deleteProperty: function(target, property) {
        /* When splice is used to remove an item, 'property' has the new length and not the index that was removed.
         * We'll use a helper key '__spliceIndex' in target, to track down the removed index.
         * In 'get' trap, when 'splice' is called, we'll flag __spliceIndex as true.
         * In 'set' trap, if target has '__spliceIndex' flagged as true, and property is a number, that number is the removed array index.
         * The exception in the above behavior is if the removed index is the last one of the array. In that case, 'set' is not called.
         * In 'delete' trap, we now know the removed array index from '__spliceIndex'.
         */

        let spliceIndex = target['__spliceIndex'];
        if (hasValue(spliceIndex)) {
          spliceIndex = Number.isInteger(spliceIndex) ? spliceIndex : property;
          target['__spliceIndex'] = undefined;
        }
        ...
        changes.push({
          type: 'delete',
          target: target,
          property: spliceIndex,
          newValue: null,
          previousValue: previousValue[spliceIndex],
          currentPath: _getPath(target, spliceIndex),
          jsonPointer: _getPath(target, spliceIndex, true),
          proxy: proxy
        });
      ...
      set: function(target, property, value, receiver) {
        if (target['__spliceIndex'] === true && !isNaN(property)) {
          target['__spliceIndex'] = Number.parseInt(property, null);
        }
        ...
        if (targetProp !== value || originalChange === false || (property === 'length' && target instanceof Array && target['__length'] !== value)) {
          ...
          if (!observable.paused) {
            if (onModelChanging !== undefined && onModelChanging !== null) {
              onModelChanging(change)
                .pipe(take(1))
                .subscribe((proceed) => {
                  if (!proceed) return;
                  ...
                  _doSet(originalChange, property, value, typeOfTargetProp, targetProp);
                });
            }
          } else {
            _doSet(originalChange, property, value, typeOfTargetProp, targetProp);
  ...
  return {
    /*	Method:
			...
			Returns:
				An ES6 Proxy object.
		*/
    create: function<T>(target: any, domDelay: boolean, modelChanging: (c: Change<T>) => Observable<boolean>, observer: ChangesCallback<T>): T {
      ...
      var proxy = _create<T>(target, domDelay, null, null);

      // assign the observer function
      if (typeof observer === 'function') this.observe(proxy, observer);

      onModelChanging = modelChanging;
    ...
    /*	Method: observe
			...
			Returns:
				Nothing.
		*/
    observe: function<T>(proxy: any, observer: ChangesCallback<T>): void {
    ...
    /*	Method: resume
			...
			Parameters:
				proxy 	- the ES6 Proxy returned by the create() method.
		*/
    resume: function(proxy: any): void {
      var i = observables.length;
      var foundMatch = false;
      while (i--) {
        if (observables[i].parentProxy === proxy) {
    ...
    /*	Method: pauseChanges
			...
			Parameters:
				proxy	- the ES6 Proxy returned by the create() method.
		 */
    pauseChanges: function(proxy: any): void {
      var i = observables.length;
      var foundMatch = false;
      while (i--) {
        if (observables[i].parentProxy === proxy) {
    ...
    /*	Method: resumeChanges
			...
			Parameters:
				proxy	- the ES6 Proxy returned by the create() method.
		 */
    resumeChanges: function(proxy: any): void {
      var i = observables.length;
      var foundMatch = false;
      while (i--) {
        if (observables[i].parentProxy === proxy) {
    ...
    /*	Method: remove
			...
			Parameters:
				proxy 	- the ES6 Proxy returned by the create() method.
		*/
    remove: function(proxy: any): void {
